...
1. Set up a new organization in GitHub (with the free plan). 

2. Navigate to https://github.com/TheRadam/pipeline-sample and fork that repository. Make sure you select the organisation you just created as the owner of the fork. 

3. Clone that repository to your machine using `git clone <path to repo>`.
...
``` yml
    - name: test
      run: dotnet test <path to .sln> --framework net8.0
```
...
> Make sure the pull request is set to be merged into the master branch of your repository, not the one you forked. 
